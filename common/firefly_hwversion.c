...
static int firefly_hwversion_adc_read_adc(ADC_CONFIG *adc){
    ...
    for(i = 0; i < 10; i++){
        ...
        udelay(5);
        while(!(read_XDATA32( adc->ctrl)&0x40)) udelay(5);
        udelay(5);
...
#define HWVERSION_ARG_SIZE 21

int firefly_hwversion_setenv(void){
	const char *prop;
    ADC_CONFIG adc;
    ...
    char hwversion_str[HWVERSION_ARG_SIZE] = {0};
    snprintf(hwversion_str, HWVERSION_ARG_SIZE, "hwversion=0x%08x", hwversion);
    env_update("bootargs", hwversion_str);
